...
@section('page-title')
    <i class="bi bi-box-seam-fill text-warning me-2"></i>Catálogo de Materiales
@endsection
...
<div class="container-fluid">
    ...
    <div class="d-flex justify-content-between align-items-center mb-4 flex-wrap gap-3">
        <div class="input-group shadow-sm" style="max-width: 450px; flex-grow: 1;">
            ...
            <button class="btn btn-outline-secondary border-start-0" id="btnLimpiarBusqueda" title="Limpiar" style="display:none;">
                ...
                <tbody id="tablaBody">
                    ...
                    <tr style="border-color: var(--border-color);">
                        ...
                        <td class="text-center">
                            <button class="btn btn-sm btn-outline-info me-1"
                                data-bs-toggle="modal" data-bs-target="#showModal{{ $material->id }}" title="Ver detalles">
                                <i class="bi bi-eye"></i>
                            </button>
                            @canEdit
                            <button class="btn btn-sm btn-outline-warning me-1"
                                data-bs-toggle="modal" data-bs-target="#editModal{{ $material->id }}" title="Editar">
                                <i class="bi bi-pencil"></i>
                            </button>
                            <button class="btn btn-sm btn-outline-danger"
                                onclick="confirmDelete({{ $material->id }}, '{{ $material->nombre }}')" title="Eliminar">
                                <i class="bi bi-trash"></i>
                            </button>
                            <form id="delete-form-{{ $material->id }}" action="{{ route('materiales.destroy', $material) }}" method="POST" style="display:none;">
                                @csrf
                                @method('DELETE')
                            </form>
                            @endcanEdit
                    ...
                    <tr id="emptyOriginal">
                        <td colspan="7" class="text-center text-muted py-5">
                            ...
                            @canEdit
                            <button class="btn btn-sm btn-warning mt-2" data-bs-toggle="modal" data-bs-target="#createModal">
            ...
            <form action="{{ route('materiales.store') }}" method="POST">
                ...
                <div class="modal-body p-4">
                    <div class="row g-3 mb-4">
                        ...
                        <div class="col-12">
                            <label class="form-label small fw-semibold text-muted">Descripción</label>
                            <textarea name="descripcion" class="form-control" rows="2" placeholder="Detalles técnicos o notas adicionales..."></textarea>
                        </div>
                    </div>
                    <div class="row g-3 mb-4">
                        <div class="col-md-4">
                    ...
                    <div class="p-3 rounded-4 bg-body-tertiary border border-dashed">
                        <p class="small text-muted mb-3 fw-semibold">
                            <i class="bi bi-info-circle me-1"></i> Podrá ajustar las cantidades en el módulo (Inventario)
                        </p>
                        <div class="row g-3">
                            <div class="col-md-4">
                                <label class="form-label small fw-semibold text-muted">Cantidad Inicial</label>
                                <input type="number" step="0.01" name="cantidad_actual" class="form-control border-success-subtle" placeholder="0.00" required>
                            </div>
                            <div class="col-md-4">
                                <label class="form-label small fw-semibold text-muted">Stock Mínimo</label>
                                <input type="number" step="0.01" name="cantidad_minima" class="form-control border-danger-subtle" placeholder="0.00" required>
                            </div>
                            <div class="col-md-4">
                                ...
                                <div class="input-group">
                                    ...
                                    <input type="number" step="0.01" name="precio_unitario" class="form-control border-success-subtle" placeholder="0.00" required>
                                </div>
                            </div>
                        </div>
                    </div>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    function buildControls(totalPages) {
        ...
        for (let p = s; p <= e; p++) {
            const pg = p, btn = makeBtn(p, false, () => { currentPage = pg; render(); });
    ...
    function makeEllipsis() {
        ...
        s.textContent = '…';
        s.style.cssText = 'padding:0 4px;opacity:.4;font-size:.85rem;line-height:32px;';
        return s;
    }
